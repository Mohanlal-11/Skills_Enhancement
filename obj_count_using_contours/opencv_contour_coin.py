...
def show(ori_img, filtered_img, binary_img, contour_img, name):
    ...
    cv.imwrite(f"./contours/{name}", contour_img)   
...
def main(main_folder, visualize=False):
    ...
    for im_path in im_paths:
        ...
        total_objects = find_objects(contours=contours, avg_area=800, im_name=im_name)
        ...
        for cont in total_objects:
            ...
            image = cv.putText(image, str(i), (int(x), int(y)), 1, 0.4, (0,255,0), 1)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        main(main_folder=main_folder, visualize=True)
